...
This project was designed to simulate a secure smart parking environment where users must enter a valid password before accessing the parking area.

The system integrates multiple embedded hardware components to provide both security and monitoring features.  
A keypad is used for password entry, while the LCD screen displays system messages and instructions.  
Servo motors simulate parking gate operation, LEDs indicate system status, and the buzzer provides warning alarms during invalid access attempts.

The project also includes a dual 7-segment display controlled using CD4511 BCD decoders to display countdown values and parking slot information.

This system demonstrates real-time embedded control, hardware interfacing, and security logic implementation using CircuitPython.
...
- LCD user interface
- Automatic servo motor gate control
- Wrong password detection
- System lockout protection mode
- Countdown timer using dual 7-segment display
- Green LED access indicator
- Red LED warning indicator
- Active buzzer alarm system
- Parking slot monitoring simulation
- Real-time hardware interaction
- Embedded system security logic
...
| 4×4 Matrix Keypad | User password input |
| LCD 16×2 I2C Display | System messages and status |
| Servo Motor | Automatic parking gate |
| Green LED | Access granted indicator |
| Red LED | Warning and alarm indicator |
| Active Buzzer | Audio alarm system |
| Dual 7-Segment Display | Countdown and slot display |
| CD4511 Decoder | BCD to 7-segment decoding |
...
The system begins by initializing all hardware peripherals including the LCD display, keypad, LEDs, buzzer, servo motor, and dual 7-segment display.

A welcome message is displayed on the LCD asking the user to enter the password.

The user enters the password through the keypad.  
Each pressed key is read and processed by the Raspberry Pi Pico W.

If the entered password is correct:

- The green LED turns ON
- The buzzer produces a short confirmation beep
- The servo motor opens the parking gate
- The LCD displays an access granted message
- The parking slot becomes accessible
...
- The 7-segment display updates the countdown value

After multiple incorrect attempts, the system enters lockout mode where:

- The LCD displays a system locked message
- The buzzer alarm repeats
- The red LED blinks
- The countdown timer starts from 10 to 00

After the countdown finishes, the system resets automatically and returns to normal operation.
...
The figure above shows the complete real hardware implementation of the smart parking security system using Raspberry Pi Pico W and external peripherals.

---

## Password Authentication Hardware

![Password Authentication](hardware_password_authentication.jpeg)

This state demonstrates the password authentication process where the user enters the password using the matrix keypad while the LCD displays instructions.

---

## Parking Slot Selection

![Parking Slot Selection](hardware_parking_slot_selection.jpeg)

The figure above demonstrates the parking slot selection process and real-time hardware response.

---

## Parking Slot Entry Opening

![Parking Slot Entry Opening](hardware_slot2_entry_opening.jpeg)

This figure shows the servo motor gate opening process after successful authentication.
...
![Wokwi Schematic](wokwi_system_schematic_diagram.png)

The complete Wokwi simulation schematic showing all system connections and embedded hardware components.

---

## Vehicle Entry Operation

![Vehicle Entry](simulation_slot1_entry_opening.png)

This simulation demonstrates successful vehicle entry and gate opening operation.

---

## Vehicle Exit Operation

![Vehicle Exit](wokwi_vehicle_exit_operation.png)

The figure above demonstrates vehicle exit operation and parking control functionality.

---

## Lockdown Mode Simulation

![Lockdown Mode](simulation_system_lockout_mode.png)

This state demonstrates the system security lockout mode after multiple incorrect password attempts.

---

## Wokwi Countdown Lockdown

![Wokwi Lockdown](wokwi_system_lockdown_mode.png)

The simulation displays the countdown process during security lockdown mode.

---

# System Flowchart

![Flowchart](smart_parking_system_flowchart.png)

The flowchart above explains the complete logic and operation sequence of the smart parking security system.
...
- Wokwi Simulator
- Raspberry Pi Pico W
- Embedded GPIO Programming

---

# Source Code

The complete CircuitPython source code for the smart parking security system is shown below:
...
The complete Open-Ended Lab (OEL) technical report is available below:
...
- Improve smart parking security
- Simulate automated parking access
- Implement embedded authentication logic
- Demonstrate hardware-software integration
- Apply real-time monitoring concepts
- Develop embedded control systems using CircuitPython
...
- WiFi remote control
- Automatic vehicle detection sensors
...
This project successfully demonstrates the implementation of a Raspberry Pi Pico W based smart parking security system using embedded system concepts and CircuitPython programming.

The system integrates authentication, security alarms, servo motor gate control, LCD interfacing, and countdown monitoring into a single embedded solution.

The project also highlights the importance of real-time hardware interaction and intelligent security logic in modern smart parking applications.
